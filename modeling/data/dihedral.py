...
def print_output(dihe_histo):
  ...
  for k in sorted(dihe_histo.keys()):
    output.write("%5s %5d %5d %14d \n"  % (k[0],k[1]*10,k[2]*10, dihe_histo[k]))
  ...
  for n in range(0,len(ss_list_tot)):
    ...
    if len(i) >= 5:
      ...
      for k in range(2,len(i)-2):
          ...
          s=i[k-2]+i[k-1]+i[k]+i[k+1]+i[k+2]
          if s in histo:
             histo[s]=histo[s]+1
          else:
             histo[s]=1
    
          v1=(xl[k-2],yl[k-2],zl[k-2])
          v2=(xl[k-1],yl[k-1],zl[k-1])
          v3=(xl[k],yl[k],zl[k])       
          v4=(xl[k+1],yl[k+1],zl[k+1])
          v5=(xl[k+2],yl[k+2],zl[k+2])
          
         
          try: td=(s,int(dihedral(v1,v2,v3,v4)/10),int(dihedral(v2,v3,v4,v5)/10))
          except: continue
